...
P8.Nipponbare.TEJ.Chr12\t12" > sv_chr_map.txt
    bcftools annotate --rename-chrs sv_chr_map.txt --threads 10 -Oz -o renamed_$vcf.gz ../raw_vcf_dir/$vcf
bcftools norm -m -both renamed_$vcf.gz  -o norm_renamed_$vcf
# filter_SV
bash ../scripts/sv_filter.sh norm_renamed_$vcf sv.$vcf
# assign ID to vcf
awk 'BEGIN{OFS="\t"} 
/^#/ {print; next} 
{
...
perl -i -ne 'print unless eof && /^[\x00\s]*$/' SV.split.$vcf
cp SV.split.$vcf ../vcf_dir/
