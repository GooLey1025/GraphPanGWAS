...
FaHC_P8_Nipponbare_TEJ#0#P8.Nipponbare.TEJ.Chr12\t12" > snp_chr_map.txt
    bcftools annotate --rename-chrs snp_chr_map.txt --threads 20 -o renamed_$vcf.gz ../raw_vcf_dir/$vcf
...
}' norm_renamed_$vcf > SNP.split.$vcf
perl -i -ne 'print unless eof && /^[\x00\s]*$/' SNP.split.$vcf
cp SNP.split.$vcf ../vcf_dir/
