...
FaHC_P8_Nipponbare_TEJ#0#P8.Nipponbare.TEJ.Chr12\t12" > INDEL_chr_map.txt
    bcftools annotate --rename-chrs INDEL_chr_map.txt --threads 20 -o renamed_$vcf ../raw_vcf_dir/$vcf
# assign ID to vcf
awk 'BEGIN{OFS="\t"} 
/^#/ {print; next} 
{
...
}' renamed_$vcf > INDEL.unsplit.$vcf
perl -i -ne 'print unless eof && /^[\x00\s]*$/' INDEL.unsplit.$vcf
cp INDEL.unsplit.$vcf ../vcf_dir/
